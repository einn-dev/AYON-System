...
const NAV_ITEMS = [
  { path: '/chairperson/dashboard',       label: 'Dashboard',        icon: '⊞' },
  { path: '/chairperson/for-endorsement', label: 'For Endorsement',  icon: '📋' },
  { path: '/chairperson/endorsed',        label: 'Endorsed',          icon: '✅' },
  { path: '/chairperson/notifications',   label: 'Notifications',     icon: '🔔' },
];
  ...
  const handleEndorse = async (decision) => {
    ...
    try {
      await API.post(`/proposals/${selected.proposal_id}/endorse`, {
        endorser_role: endorserRole, endorse_status: decision, remarks,
      });
      ...
      {!selected ? (
        <div className="panel">
          ...
          <table className="data-table">
            <thead>
              <tr><th>#</th><th>Researcher</th><th>Title</th><th>Type</th><th>Status</th><th>Action</th></tr>
            </thead>
            <tbody>
              {pending.map((p, i) => (
        ...
        <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 300px', gap: 20 }}>
            ...
            <div className="panel">
              <div className="panel-header"><h2>Your Endorsement</h2></div>
              <div className="panel-body">
                <div className="form-group">
                  <label>Remarks</label>
                  <textarea rows={5} value={remarks} onChange={e => setRemarks(e.target.value)}
                    placeholder="Add remarks..." style={{ resize: 'vertical' }} />
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  <button className="btn btn-success" disabled={saving}
                    onClick={() => handleEndorse('endorsed')}>
                    {saving ? 'Processing...' : '✓ Endorse'}
                  </button>
      ...
      <div className="panel">
        <table className="data-table">
          ...
          <tbody>
            ...
            {endorsed.length === 0 && (
              <tr><td colSpan={4}><div className="empty-state"><p>No endorsed proposals.</p></div></td></tr>
            )}
          </tbody>
        </table>
      </div>
        ...
        {notifs.length === 0 ? <div className="empty-state"><p>No notifications.</p></div>
          : notifs.map(n => (
            <div key={n.notif_id} onClick={() => !n.is_read && markRead(n.notif_id)}
              style={{ padding: '14px 20px', borderBottom: '1px solid #f0f4f8',
...
const Overview = ({ proposals }) => (
  <>
    <div className="page-header">
      ...
      <p>Review and endorse research proposals from your department.</p>
    </div>
...
const ChairpersonDashboard = () => {
  ...
  return (
    <Layout navItems={NAV_ITEMS} role="Chairperson">
      <Routes>
        <Route index element={<Navigate to="dashboard" replace />} />
        <Route path="dashboard" element={<Overview proposals={proposals} />} />
        <Route path="for-endorsement" element={
          <>
            <div className="page-header"><h1>For Endorsement</h1></div>
            <EndorsementPanel proposals={proposals} endorserRole="chairperson" onRefresh={fetchProposals} />
          </>
        } />
